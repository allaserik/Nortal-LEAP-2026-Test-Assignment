...
class ApiRulesIntegrationTest {
    ...
    void cannotBorrowAlreadyLoanedBook() {
        ResultResponse borrow1 = rest.postForObject(url("/api/borrow"), new BorrowRequest("b1", "m1"),
                ResultResponse.class);
        assertNotNull(borrow1);
        assertTrue(borrow1.ok());

        ResultResponse borrow2 = rest.postForObject(url("/api/borrow"), new BorrowRequest("b1", "m2"),
                ResultResponse.class);
    ...
    void returnRejectedWhenNotCurrentBorrower() {
        ...
        ResultWithNextResponse wrongReturn = rest.postForObject(
        ...
        BookResponse book = rest.getForObject(url("/api/books"), BooksResponse.class).items().stream()
    ...
    void reserveAvailableBookLoansImmediately() {
        ResultResponse reserved = rest.postForObject(url("/api/reserve"), new ReserveRequest("b3", "m2"),
                ResultResponse.class);
    ...
    void duplicateReservationIsRejected() {
        ...
        ResultResponse r1 = rest.postForObject(url("/api/reserve"), new ReserveRequest("b4", "m2"),
                ResultResponse.class);
        assertNotNull(r1);
        assertTrue(r1.ok());

        ResultResponse r2 = rest.postForObject(url("/api/reserve"), new ReserveRequest("b4", "m2"),
                ResultResponse.class);
        assertNotNull(r2);
